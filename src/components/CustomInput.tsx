...
export default function CustomInput({placeholder,isHidden = false}:CustomInputTypes) {
    return(
        <div className='w-full rounded-xl text-xl text-text-dark border border-input-border bg-input-bg flex items-center justify-between font-semibold relative'>
            <input className={`w-full text-input-text h-15 ${isHidden ? 'pl-5 pr-18' : 'px-5'}`} placeholder={placeholder}/>
            {isHidden ?
